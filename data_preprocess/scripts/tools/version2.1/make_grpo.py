...
def _normalize_system(msg):
    if not msg or msg.get("role") != "system":
        return None
...
def _parse_children_names_from_assistant_old(gt_text: str) -> list[str]:
    s = (gt_text or "").strip()
    if not s or s == NO_INTERACTION_STR or s == "[]":
        return []
...
def _iter_potential_blocks_from_content(user_content_str: str):
    ...
    for part in parts:
        part = part.strip()
        if not part or part == PSEP_TOKEN:
            continue
        try:
            blk = json.loads(part)
...
def _extract_node_depth_from_user_content(user_content_str: str) -> int:
    """
    从 user.content 中的 <POTENTIAL_SPANS> 块解析 depth 字段。
    """
    for blk in _iter_potential_blocks_from_content(user_content_str):
        try:
            return int(blk.get("depth", 0))
        except Exception:
            return 0
    return 0
...
def _parse_chunk_from_sample_id(sample_id: str) -> dict:
    ...
    if m:
        out["record_id"] = m.group("rec")
        out["root_user"] = m.group("root")
        try:
            out["chunk_index"] = int(m.group("idx"))
        except Exception:
...
def _compose_sft_chunk_info(row: pd.Series, messages: list[dict]) -> dict:
    ...
    try:
        ...
        try:
            ...
            sample_id = None
    parsed_id = _parse_chunk_from_sample_id(str(sample_id) if sample_id is not None else "")
    ...
    record_id = meta_msg.get("record_id") or meta_col.get("record_id") or parsed_id.get("record_id")
    root_user = meta_msg.get("root_user") or meta_col.get("root_user") or parsed_id.get("root_user")

    chunk_index = meta_msg.get("chunk_index")
    if chunk_index is None:
        chunk_index = meta_col.get("potential_chunk_index")
    if chunk_index is None:
        chunk_index = parsed_id.get("chunk_index")

    chunk_size = meta_msg.get("chunk_size")
    if chunk_size is None:
        chunk_size = meta_col.get("k_per_chunk")

    first_layer_users = meta_msg.get("first_layer_users")
    ...
    out = {
        ...
        "first_layer_users": first_layer_users,
    }

    if "node_depth" in getattr(row, "index", []):
        try:
            out["node_depth"] = int(row["node_depth"])
        except Exception:
            pass
...
# ---------- 构造 cond_gt_by_turn ----------
def _build_strict_parent_cond_gt(pairs: list[tuple[dict, dict]],
                                 *,
                                 enforce_children_in_potential: bool = False) -> list[list[dict]]:
    ...
    for (u, a) in pairs:
        ...
        child_names = _parse_positive_children_from_new_assistant(a.get("content", "") or "",
    ...
    for d, p2c in depth_parent2children.items():
        if d < 0 or d >= L:
            continue
        if not p2c:
            cond_gt_by_turn[d] = []
        else:
            cond_gt_by_turn[d] = [{parent: list(children)} for parent, children in p2c.items()]
...
def _build_grpo_for_one_file(
    ...
    *,
    ...
    samples = []
    seen = set()
    kept, drop_no_root, drop_no_pairs = 0, 0, 0
    ...
    for ridx, row in tqdm(df.iterrows(), total=len(df), desc=f"Build GRPO ({os.path.basename(sft_parquet_path)})"):
        ...
        if allow_system_anywhere:
            for mi in messages:
                if isinstance(mi, dict) and mi.get("role") == "system":
                    system_msg = _normalize_system(mi)
                    break
        else:
        ...
        if "node_depth" in getattr(row, "index", []):
            try:
                node_depth_val = row.get("node_depth")
            except Exception:
                node_depth_val = None
        if node_depth_val is None or pd.isna(node_depth_val):
            node_depth_val = _extract_node_depth_from_user_content(root_user_raw)
        try:
            node_depth = int(node_depth_val)
        except Exception:
            node_depth = 0
        if node_depth < 0:
            node_depth = 0

        # >>> 修改点 2：**只保留根节点（depth == 0）**
        if node_depth != 0:
            continue

        try:
            root_user_json = json.loads(root_user_raw) if isinstance(root_user_raw, str) else None
        except Exception:
            root_user_json = None
        ...
        cond_gt_by_turn = _build_strict_parent_cond_gt(
            pairs, enforce_children_in_potential=enforce_children_in_potential
        )
        any_child = any(len(layer) > 0 and any(len(next(iter(d.values()))) > 0 for d in layer)
                        for layer in cond_gt_by_turn)
        if (not keep_all_no_interact) and (not any_child):
            continue
        ...
        dmin = min((p.get("depth", 0) for p in root_pots_full), default=node_depth) if has_psep_block else node_depth
        dmax = max((p.get("depth", 0) for p in root_pots_full), default=node_depth) if has_psep_block else node_depth
        ...
        sft_chunk_info.setdefault("node_depth", int(node_depth))
    ...
    print(f"Dropped non-root samples : {len(df) - kept - drop_no_pairs - drop_no_root}")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description=(
            "Build GRPO dataset (VAL only) — read NEW SFT (with <POTENTIAL_SPANS> & coverage array 0/1/2); "
            "output reward_model.ground_truth.cond_gt_by_turn and root_potential meta. "
            "（本版仅保留 node_depth == 0 的根节点样本，但保留完整 cond_gt_by_turn）"
        )
    )
